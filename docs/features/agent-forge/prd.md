...
13. 提供 `doctor` 命令执行三层环境诊断：核心依赖（docker、curl、git）→ 运行时（docker daemon 运行状态、权限）→ 可选工具（jq、buildx），并使用 apt-get/dnf/yum/brew 自动安装缺失核心依赖
...
| `doctor` 检测到缺失依赖后自动安装 | 缺失的 curl/git 等依赖被安装成功，重新 `doctor` 全部通过 |
...
```mermaid
flowchart LR
    A([执行 doctor]) --> B[检测核心依赖<br/>docker/curl/git]
    B --> C{全部满足？}
...
- **Docker Engine**（版本 >= 20.10）— 所有容器化操作的基础，`build`、`run`、`export`、`import` 命令均依赖 Docker daemon 运行
- **构建好的 Docker 镜像** — `run`、`deps` 命令依赖 `build` 先成功执行生成镜像
- **Git 仓库远程 URL 或 UPDATE_URL** — `update` 命令需要能够访问更新源以下载新版本
- **LLM provider API 网络的通性** — `endpoint test` 需要能够连接到对应 provider 的 API 地址；`endpoint apply` 虽然不实时连接但需要有效的 URL 格式
- **国内镜像源可用性** — 构建过程中 npm 映射 npmmirror、pip 映射 aliyun、yum 映射 aliyun centos vault，这些源需要可访问；构建也支持通过 `--gh-proxy` 代理 GitHub 资源
- **Linux x86_64 内核兼容性** — 基础镜像和 agent 二进制文件均为 linux-amd64 架构，宿主内核需支持容器运行时所需的 cgroup、namespace 等特性
- **BuildKit 支持** — `build` 命令检测 `DOCKER_BUILDKIT` 环境变量以启用 BuildKit 构建加速
- **curl 和 git 命令行工具** — `build` 过程中可能依赖 curl 下载资源，`update` 依赖 git 获取仓库信息
