...
定义项目的 GoReleaser 多平台构建配置规范——构建矩阵、ldflags 注入、归档格式和包管理器生成。
...
- **THEN** 新版本 SHALL 覆盖旧版本，二进制替换为最新版
...
- **THEN** `/usr/bin/agent-forge` SHALL 被移除
...
- **THEN** `cmd/version.go` 中 SHALL 存在对应的变量声明
