...
`agent-forge build` 命令 SHALL 调用构建引擎执行实际的 Docker 镜像构建，在构建过程中实时流式输出构建日志到 stdout，而非构建完成后一次性输出。

#### Scenario: Build with all deps and custom base image

- **WHEN** 用户执行 `agent-forge build -b docker.1ms.run/ubuntu:22.04 -d all`
- **THEN** 系统 SHALL 解析参数，生成 Dockerfile，创建构建上下文，调用 Docker Engine API 构建镜像，并在构建过程中实时输出构建日志

#### Scenario: Build with default parameters

- **WHEN** 用户执行 `agent-forge build`（无任何参数）
- **THEN** 系统 SHALL 使用默认基础镜像 `docker.1ms.run/centos:7` 和空依赖列表执行构建，实时输出构建日志

#### Scenario: Build with rebuild mode

- **WHEN** 用户执行 `agent-forge build -R`
- **THEN** 系统 SHALL 使用临时标签构建，成功后原子替换 `agent-forge:latest` 标签，构建过程中实时输出日志

#### Scenario: Build with no-cache flag

- **WHEN** 用户执行 `agent-forge build --no-cache`
- **THEN** 系统 SHALL 在 ImageBuild API 调用中设置 `NoCache: true`，强制跳过 Docker 构建缓存，构建过程中实时输出日志

#### Scenario: Build with GitHub proxy

- **WHEN** 用户执行 `agent-forge build --gh-proxy https://ghproxy.net`
- **THEN** 系统 SHALL 将代理 URL 传递给 Dockerfile 生成器，用代理包装 GitHub 下载 URL，构建过程中实时输出日志
...
#### Scenario: Build interrupted by Ctrl+C during streaming

- **WHEN** 用户在构建输出流式打印期间按 Ctrl+C
- **THEN** 系统 SHALL 在 1 秒内停止 Docker 构建，输出已收集的日志，以非零退出码退出
