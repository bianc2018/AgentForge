...
description: "当用户明确需要完整的 OpenSpec 加 Superpowers 路径（从澄清到 proposal、design、tasks、实现、验证以及可选的归档）时使用。"
model_hint: sonnet
tags:
  - orchestration
  - openspec
...
使用此 skill 作为 feature 交付的团队入口点。它协调工作顺序，不替代详细的 OpenSpec 或 Superpowers 子 skill。
...
4. 如果项目使用 OpenSpec 归档流程且代码、specs 和验证一致，则归档变更作为 OpenSpec 的最后一步。
5. 在存在验证证据之前，不要声称完成。
...
- 用户明确要求先 brainstorm，然后 proposal/design/tasks，再是实现，最后验证
- 仓库策略明确要求此工作流
...
- 代码、测试和全新的验证输出
- 当归档流程是项目工作流的一部分时，已归档的 OpenSpec 变更
- 当存在 `.superpowers-memory/` 时，更新后的 Superpowers memory
...
**如何使用：**
1. 在设计批准后（步骤 1），使用 `TeamCreate` 建立团队
...
- 在设计批准之前不要开始实现
- 对行为变更不要跳过 OpenSpec 产物
- 在代码、测试和 specs 一致之前不要归档变更
- 当请求包含 worktree、TDD 或验证时不要跳过它们
- 保持 skill 可移植：使用仓库本地路径，避免机器特定的假设
