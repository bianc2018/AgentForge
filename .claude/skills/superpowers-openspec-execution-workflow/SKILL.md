...
description: "当团队明确希望为 feature 使用 Superpowers 探索、OpenSpec 规格化、Superpowers 执行和 OpenSpec 归档工作流时使用"
model_hint: sonnet
tags:
  - orchestration
  - openspec
...
当团队想要以下四步交付路径时使用此 skill：

1. 使用 Superpowers 探索
2. 使用 OpenSpec 锁定变更
3. 使用 Superpowers 执行，完成实现、测试和验证
4. 归档已完成的 OpenSpec 变更

此 skill 是一个协调器。它应将细节工作委托给现有的工作流 skill，而不是重复它们。

这是一个明确的 opt-in 工作流。默认情况下不要使用。仅在用户明确要求此工作流、命名了此 skill、或仓库策略明确要求时使用。

如果仓库中存在 `.superpowers-memory/`，在开始时读取 `PROJECT_CONTEXT.md`、`CURRENT_STATE.md`、`DECISIONS.md`、`KNOWN_FAILURES.md`、`VERIFICATION_BASELINE.md`、`TEAM_PREFERENCES.md`、`USER_PROFILE.md`、`AGENT_NOTES.md` 和最新的会话日志条目，然后在最终归档前更新相关文件，以便下次会话可以使用真实上下文继续。
...
4. 如果验证后实现和 specs 一致，使用 `$openspec-archive-change` 归档已完成的变更。
5. 如果 `.superpowers-memory/` 存在，在验证和归档决策后执行内存一致性检查：确保持久性事实、当前状态、决策、失败模式和会话结果反映在正确的文件中。
6. 当想用一个命令来审查清单、获取更新建议并可选地在执行或归档工作后运行验证时，优先使用 `scripts/run-superpowers-memory-closeout.ps1`。
...
当安装了 oh-my-claudecode 且任务足够大能从并行 agent 中受益时，可使用 OMC 的 `/team` 来加速执行。这是可选的 — 顺序单 agent 执行始终有效。
...
**如何使用：**
1. 在设计锁定后（步骤 2），使用 `TeamCreate` 建立团队
2. 通过 `TaskCreate` 将实现计划分解为具有依赖链的任务
3. 通过 `Agent` 工具使用 `team_name` 生成团队成员 — 使用如 `executor`、`verifier`、`critic` 等角色
4. 团队负责人（你）进行协调；团队成员执行任务并通过 `SendMessage` 报告
5. 所有任务完成后，使用 `TeamDelete` 进行清理
...
- 在探索阶段不要创建实现代码。
- 在所需的 OpenSpec 产物完成之前不要开始编码。
- 在有全新的验证输出之前不要声称成功。
- 在代码、测试和 specs 一致之前不要归档变更。
- 当 `.superpowers-memory/` 存在时，不要让内存与最终归档决策不同步。

## 何时使用

- 用户明确要求"先探索，再规格化，最后执行"
...
- 代码、测试和全新的验证证据
- 当使用内存时，更新后的 Superpowers memory 和内存验证证据
- 当使用了结算辅助工具时，可选的结算辅助输出
- 当工作完成时，已归档的 OpenSpec 变更
