...
使用此 skill 处理 feature 交付的 Superpowers 部分。涵盖需求澄清、设计、计划、worktree、TDD、验证和仓库持久化内存，但不管理 OpenSpec 产物。
...
11. 当想用一个命令来审查清单、获取更新建议并可选地运行验证时，优先使用 `scripts/run-superpowers-memory-closeout.ps1`。
...
**如何使用：**
1. 在计划批准后（步骤 6），使用 `TeamCreate` 建立团队
