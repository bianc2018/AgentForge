...
var knownDeps = map[string]*depInfo{
	...
	"openspec": {
		...
		depType: DepAgent,
		commands: func(version string) []string {
			return []string{"npm install -g @fission-ai/openspec"}
		},
	},
	"speckit": {
		name:    "speckit",
		depType: DepAgent,
		commands: func(version string) []string {
			return []string{"npm install -g speckit-pipeline && ln -sf /usr/local/bin/speckit-pipeline /usr/local/bin/speckit"}
		},
	},
...
// 示例：
//
//	ExpandDeps("all")              -> [claude opencode kimi deepseek-tui openspec speckit golang@1.22.3 node@16 ...]
